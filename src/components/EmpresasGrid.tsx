...
import type { EmpresaAgregada } from '@/types/contracts'
...
export function EmpresasGrid({ empresas, searchTerm }: Props) {
  const filtered = searchTerm
    ? empresas.filter(e =>
        `${e.nome} ${e.itens.map(i => i.objeto).join(' ')}`.toLowerCase().includes(searchTerm.toLowerCase())
      )
    : empresas
  ...
  return (
    <div className="flex flex-col gap-3">
      {filtered.map((emp, i) => (
        <EmpresaCard key={emp.nome} emp={emp} index={i} searchTerm={searchTerm} />
      ))}
    </div>
  )
}

function EmpresaCard({ emp, index, searchTerm }: { emp: EmpresaAgregada; index: number; searchTerm: string }) {
  const [open, setOpen] = useState(false)
  const p = PALETTES[index % PALETTES.length]
  const isOpen = open || searchTerm.length > 0
    ...
    <div className={`bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm overflow-hidden border-l-4 ${p.border}`}>
      ...
      >
        <div className="flex items-center gap-3 flex-1 min-w-0">
          ...
          <div className="min-w-0">
            <p className="font-semibold text-sm text-gray-900 dark:text-gray-100 truncate">{decodeHtml(emp.nome)}</p>
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">{emp.contratos} contrato{emp.contratos !== 1 ? 's' : ''}</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-5 flex-shrink-0">
          <div className="hidden sm:flex flex-col items-end gap-1.5">
            <div className="w-24 h-1.5 bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden">
              <div className={`h-full rounded-full ${p.bar}`} style={{ width: `${Math.min(emp.percentagem, 100)}%` }} />
            </div>
            <p className={`text-xs font-semibold ${p.text}`}>{emp.percentagem}%</p>
          </div>
          <div className="text-right">
            <p className="font-bold text-sm text-gray-900 dark:text-gray-100">{formatCurrency(emp.valor_total)}</p>
          </div>
          <svg
      ...
      {isOpen && (
        <div className="border-t border-gray-100 dark:border-gray-800">
          {emp.itens.sort((a, b) => b.valor - a.valor).map((item, i) => (
            <div
              key={item.id_base || i}
            ...
            >
              <div className="min-w-0">
                ...
                <span className={`inline-block text-xs font-semibold px-2 py-0.5 rounded-md mt-1.5 ${p.light} ${p.text}`}>
                  {item.categoria}
                </span>
              </div>
